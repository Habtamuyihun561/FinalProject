...
//start of free auction


    public function freeAuctionblog(){
        // $post=Auction::query();
        $post=Auction::where('status','active')->where('type','free')->get();
        ...
        if(!empty($_GET['show'])){
            $post=$post->where('status','active')->where('type','free')->orderBy('id','DESC')->paginate($_GET['show']);
        }
        else{
    ...
    public function freeblogByCategory(Request $request){
        ...
        $rcnt_post=Post::where('status','active')->where('type','free')->orderBy('id','DESC')->limit(3)->get();
...
//end of free auction
    ...
    public function blog(){
        // $post=Auction::query();
        $post=Auction::where('status','active')->where('type','premium')->get();
        ...
        if(!empty($_GET['show'])){
            $post=$post->where('status','active')->where('type','premium')->orderBy('id','DESC')->paginate($_GET['show']);
        }
        else{
            // $post=$post->where('status','active')->where('type','premium')->orderBy('id','DESC')->paginate(9);
        }
    ...
    public function registerSubmit(Request $request){
      ...
      if($tin){
        ...
        return "tin number doese not exist";
